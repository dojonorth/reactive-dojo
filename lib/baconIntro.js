...
/**
 * FOUR. Merge.
 * The real power of Bacon.JS comes in manipulating and combining streams. A  
 * really useful method for doing this is `merge`. It is called from a stream &
 * takes another stream as its only parameter. The output of which is a single
 * stream receiving values from two inputs.
 * 
 * Create a new stream from a button which will decrement the above counter. 
 * Combine with the first stream using `merge` into a single scan method.
 * 
 * You will need to write your new code alongside the answer for THREE. There 
 * is no stub code for this answer, the ID of the subtract button is 
 * `#subtract`. That is all you should need to know to complete this exercise.
 */ 
